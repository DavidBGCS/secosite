...
function normaliseSiteFile(data: SiteFile): SiteFile {
  return {
    ...data,
...
export function useFirestoreSite() {
  ...
  const siteFile = useMemo<SiteFile | undefined>(() => {
    if (!siteFileId) return undefined;
  ...
  const updateSite = async (next: SiteFile) => {
    const safe: SiteFile = {
      ...next,
