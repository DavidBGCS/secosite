...
export function useSites() {
    ...
    const addSite = async (siteFile) => {
        try {
            const siteId = siteFile.metadata.siteFileId;
            if (!siteId)
                throw new Error("Missing siteFileId");
    ...
    const saveSite = async (siteFile) => {
        try {
            ...
            await setDoc(docRef, cleaned, { merge: true });
            setSites((prev) => prev.map((s) => (s.metadata.siteFileId === siteId ? cleaned : s)));
        }
        catch (err) {
            throw new Error(err instanceof Error ? err.message : "Failed to save site");
        }
    };
    const updateSite = async (siteFile) => {
