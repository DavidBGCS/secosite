...
function inferReference(loop, address, fallback) {
    const l = clean(loop);
...
function inferCategory(assetType, tags) {
    const t = normalizeText(assetType);
    const all = [t, ...tags.map(normalizeText)].join(" ");
    if (all.includes("optical"))
        return "optical-detector";
    if (all.includes("heat"))
        return "heat-detector";
    if (all.includes("multisensor") || all.includes("multi sensor"))
        return "multisensor";
    if (all.includes("beam"))
        return "beam";
    if (all.includes("aspirating") || all.includes("asd"))
        return "aspirating";
    if (all.includes("detector"))
        return "detector";
    if (all.includes("manual call point") || all.includes("mcp"))
        return "mcp";
    if (all.includes("sounder beacon"))
        return "sounder-beacon";
    if (all.includes("sounder"))
        return "sounder";
    if (all.includes("interface"))
        return "interface";
    if (all.includes("i/o") || all.includes(" io ") || all.includes("input module") || all.includes("output module")) {
        return "io";
    }
    if (all.includes("psu") || all.includes("power supply"))
        return "psu";
    if (all.includes("panel"))
        return "panel";
    if (all.includes("repeater"))
        return "repeater";
    if (all.includes("atex"))
        return "atex";
    if (all.includes("void"))
        return "void";
    if (all.includes("attic"))
        return "attic";
    return "other";
}
function countsTowardAutoDetectionPercentage(category) {
...
function autoTags(assetType, description, locationText) {
    ...
    if (text.includes("roof"))
        tags.add("roof");
    if (text.includes("atex"))
        tags.add("atex");
    if (text.includes("plant"))
        tags.add("plantroom");
    if (text.includes("interface"))
        tags.add("interface");
    if (text.includes("i/o") || text.includes("input module") || text.includes("output module"))
        tags.add("io");
    if (text.includes("sounder"))
        tags.add("sounder");
    if (text.includes("detector"))
        tags.add("detector");
    if (text.includes("mcp") || text.includes("manual call point"))
        tags.add("mcp");
...
export function buildAssetRecord(row, ctx) {
...
export function importAssetRows(rows, ctx) {
    const warnings = [];
    const errors = [];
    const assets = [];
    ...
    rows.forEach((row, index) => {
        try {
            if (!clean(row.reference) && !(clean(row.loop) && clean(row.address))) {
                warnings.push(`Row ${index + 1}: no explicit reference found, generated from available data.`);
            }
...
export function parseBulkAddText(input, ctx) {
    const warnings = [];
    const errors = [];
    const lines = input
        .split(/\r?\n/g)
...
export function parseCsvText(csvText, mapping, ctx) {
    const lines = csvText
        .split(/\r?\n/g)
        .map((line) => line.trim())
        .filter(Boolean);
    ...
    const getMappedValue = (values, headerName) => {
        if (!headerName)
            return undefined;
...
export function parsePastedTable(input, ctx) {
    const lines = input
        .split(/\r?\n/g)
        .map((line) => line.trim())
        .filter(Boolean);
...
export function getSuggestedCsvMapping(headers) {
    const normalized = headers.map((h) => h.trim());
    const pick = (...candidates) => normalized.find((header) => candidates.some((candidate) => header.toLowerCase() === candidate.toLowerCase()));
    return {
...
export function mergeImportedAssets(existing, imported, mode = "append") {
    if (mode === "append") {
