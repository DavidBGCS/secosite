// src/core/builder/segpackQueries.ts

import type { Segpack } from "../types/segpack";
import type { FaultRecord, RemedialAction } from "../types/fault";
import type { PhotoRecord } from "../types/photo";
import type { MarkupRecord } from "../types/markup";
import type { ReplacementRecord } from "../types/replacement";
import type { ComplianceRecord } from "../types/compliance";
import type { PartRecord } from "../types/part";
import type {
  InstalledSystem,
  SystemCircuit,
  SystemDevice,
  SystemZone,
  SystemNetworkNode,
  SystemPowerSource,
} from "../types/system";

function ensureArray<T>(value: T[] | undefined): T[] {
  return Array.isArray(value) ? value : [];
}

function uniqueById<T extends { id: string }>(items: T[]): T[] {
  const seen = new Set<string>();
  const out: T[] = [];

  for (const item of items) {
    if (!item?.id || seen.has(item.id)) continue;
    seen.add(item.id);
    out.push(item);
  }

  return out;
}

function includesId(list: string[] | undefined, id: string): boolean {
  return Array.isArray(list) && list.includes(id);
}

export function getManifest(pack: Segpack) {
  return pack.manifest;
}

export function getSite(pack: Segpack) {
  return pack.site;
}

export function getSystems(pack: Segpack): InstalledSystem[] {
  return ensureArray(pack.systems);
}

export function getFaults(pack: Segpack): FaultRecord[] {
  return ensureArray(pack.faults);
}

export function getParts(pack: Segpack): PartRecord[] {
  return ensureArray(pack.parts);
}

export function getReplacements(pack: Segpack): ReplacementRecord[] {
  return ensureArray(pack.replacements);
}

export function getComplianceRecords(pack: Segpack): ComplianceRecord[] {
  return ensureArray(pack.compliance);
}

export function getPhotos(pack: Segpack): PhotoRecord[] {
  return ensureArray(pack.photos);
}

export function getMarkups(pack: Segpack): MarkupRecord[] {
  return ensureArray(pack.markups);
}

export function getSystemById(
  pack: Segpack,
  systemId: string
): InstalledSystem | undefined {
  return getSystems(pack).find((item) => item.id === systemId);
}

export function getFaultById(
  pack: Segpack,
  faultId: string
): FaultRecord | undefined {
  return getFaults(pack).find((item) => item.id === faultId);
}

export function getPartById(
  pack: Segpack,
  partId: string
): PartRecord | undefined {
  return getParts(pack).find((item) => item.id === partId);
}

export function getReplacementById(
  pack: Segpack,
  replacementId: string
): ReplacementRecord | undefined {
  return getReplacements(pack).find((item) => item.id === replacementId);
}

export function getComplianceById(
  pack: Segpack,
  complianceId: string
): ComplianceRecord | undefined {
  return getComplianceRecords(pack).find((item) => item.id === complianceId);
}

export function getPhotoById(
  pack: Segpack,
  photoId: string
): PhotoRecord | undefined {
  return getPhotos(pack).find((item) => item.id === photoId);
}

export function getMarkupById(
  pack: Segpack,
  markupId: string
): MarkupRecord | undefined {
  return getMarkups(pack).find((item) => item.id === markupId);
}

export function getSystemsByDiscipline(
  pack: Segpack,
  discipline: InstalledSystem["discipline"]
): InstalledSystem[] {
  return getSystems(pack).filter((item) => item.discipline === discipline);
}

export function getFaultsBySystemId(
  pack: Segpack,
  systemId: string
): FaultRecord[] {
  return getFaults(pack).filter((item) => item.systemId === systemId);
}

export function getFaultsByDeviceId(
  pack: Segpack,
  deviceId: string
): FaultRecord[] {
  return getFaults(pack).filter((item) => item.deviceId === deviceId);
}

export function getFaultsByStatus(
  pack: Segpack,
  status: FaultRecord["status"]
): FaultRecord[] {
  return getFaults(pack).filter((item) => item.status === status);
}

export function getFaultsBySeverity(
  pack: Segpack,
  severity: FaultRecord["severity"]
): FaultRecord[] {
  return getFaults(pack).filter((item) => item.severity === severity);
}

export function getPhotosForFault(
  pack: Segpack,
  faultId: string
): PhotoRecord[] {
  const fault = getFaultById(pack, faultId);
  if (!fault) return [];

  const linkedByIds = getPhotos(pack).filter((photo) =>
    includesId(fault.photoIds, photo.id)
  );

  const linkedByRefs = getPhotos(pack).filter((photo) => photo.refs?.faultId === faultId);

  return uniqueById([...linkedByIds, ...linkedByRefs]);
}

export function getMarkupsForFault(
  pack: Segpack,
  faultId: string
): MarkupRecord[] {
  const fault = getFaultById(pack, faultId);
  if (!fault) return [];

  const linkedByIds = getMarkups(pack).filter((markup) =>
    includesId(fault.markupIds, markup.id)
  );

  const linkedByRefs = getMarkups(pack).filter(
    (markup) => markup.refs?.faultId === faultId || markup.subjectId === faultId
  );

  return uniqueById([...linkedByIds, ...linkedByRefs]);
}

export function getPartsForFault(
  pack: Segpack,
  faultId: string
): PartRecord[] {
  const fault = getFaultById(pack, faultId);
  if (!fault) return [];

  return uniqueById(
    getParts(pack).filter((part) => includesId(fault.partIds, part.id))
  );
}

export function getReplacementsForFault(
  pack: Segpack,
  faultId: string
): ReplacementRecord[] {
  const fault = getFaultById(pack, faultId);
  if (!fault) return [];

  const linkedByIds = getReplacements(pack).filter((replacement) =>
    includesId(fault.replacementIds, replacement.id)
  );

  const linkedByRef = getReplacements(pack).filter(
    (replacement) => replacement.faultId === faultId
  );

  return uniqueById([...linkedByIds, ...linkedByRef]);
}

export function getComplianceForFault(
  pack: Segpack,
  faultId: string
): ComplianceRecord[] {
  return uniqueById(
    getComplianceRecords(pack).filter(
      (item) =>
        item.subject?.faultId === faultId || includesId(item.faultIds, faultId)
    )
  );
}

export function getPhotosForCompliance(
  pack: Segpack,
  complianceId: string
): PhotoRecord[] {
  const compliance = getComplianceById(pack, complianceId);
  if (!compliance) return [];

  const linkedByIds = getPhotos(pack).filter((photo) =>
    includesId(compliance.photoIds, photo.id)
  );

  const linkedByRefs = getPhotos(pack).filter(
    (photo) => photo.refs?.complianceId === complianceId
  );

  return uniqueById([...linkedByIds, ...linkedByRefs]);
}

export function getMarkupsForCompliance(
  pack: Segpack,
  complianceId: string
): MarkupRecord[] {
  const compliance = getComplianceById(pack, complianceId);
  if (!compliance) return [];

  const linkedByIds = getMarkups(pack).filter((markup) =>
    includesId(compliance.markupIds, markup.id)
  );

  const linkedByRefs = getMarkups(pack).filter(
    (markup) => markup.refs?.complianceId === complianceId
  );

  return uniqueById([...linkedByIds, ...linkedByRefs]);
}

export function getComplianceForSystem(
  pack: Segpack,
  systemId: string
): ComplianceRecord[] {
  return getComplianceRecords(pack).filter(
    (item) => item.subject?.systemId === systemId
  );
}

export function getComplianceForDevice(
  pack: Segpack,
  deviceId: string
): ComplianceRecord[] {
  return getComplianceRecords(pack).filter(
    (item) => item.subject?.deviceId === deviceId
  );
}

export function getPhotosForSystem(
  pack: Segpack,
  systemId: string
): PhotoRecord[] {
  return getPhotos(pack).filter((photo) => photo.refs?.systemId === systemId);
}

export function getPhotosForDevice(
  pack: Segpack,
  deviceId: string
): PhotoRecord[] {
  return getPhotos(pack).filter((photo) => photo.refs?.deviceId === deviceId);
}

export function getMarkupsForPhoto(
  pack: Segpack,
  photoId: string
): MarkupRecord[] {
  const directRefs = getMarkups(pack).filter((markup) => markup.refs?.photoId === photoId);

  const reverseRefs = getMarkups(pack).filter((markup) => {
    const photo = getPhotoById(pack, photoId);
    return includesId(photo?.refs?.markupIds, markup.id);
  });

  return uniqueById([...directRefs, ...reverseRefs]);
}

export function getPhotoForMarkup(
  pack: Segpack,
  markupId: string
): PhotoRecord | undefined {
  const markup = getMarkupById(pack, markupId);
  if (!markup?.refs?.photoId) return undefined;
  return getPhotoById(pack, markup.refs.photoId);
}

export function getSystemDevices(
  pack: Segpack,
  systemId: string
): SystemDevice[] {
  return ensureArray(getSystemById(pack, systemId)?.devices);
}

export function getSystemDeviceById(
  pack: Segpack,
  systemId: string,
  deviceId: string
): SystemDevice | undefined {
  return getSystemDevices(pack, systemId).find((item) => item.id === deviceId);
}

export function getAllDevices(pack: Segpack): SystemDevice[] {
  return getSystems(pack).flatMap((system) => ensureArray(system.devices));
}

export function getDeviceById(
  pack: Segpack,
  deviceId: string
): SystemDevice | undefined {
  return getAllDevices(pack).find((item) => item.id === deviceId);
}

export function getSystemCircuits(
  pack: Segpack,
  systemId: string
): SystemCircuit[] {
  return ensureArray(getSystemById(pack, systemId)?.circuits);
}

export function getSystemZones(
  pack: Segpack,
  systemId: string
): SystemZone[] {
  return ensureArray(getSystemById(pack, systemId)?.zones);
}

export function getSystemNetworkNodes(
  pack: Segpack,
  systemId: string
): SystemNetworkNode[] {
  return ensureArray(getSystemById(pack, systemId)?.networkNodes);
}

export function getSystemPowerSources(
  pack: Segpack,
  systemId: string
): SystemPowerSource[] {
  return ensureArray(getSystemById(pack, systemId)?.powerSources);
}

export function getDeviceFaults(
  pack: Segpack,
  deviceId: string
): FaultRecord[] {
  return getFaultsByDeviceId(pack, deviceId);
}

export function getDevicePhotos(
  pack: Segpack,
  deviceId: string
): PhotoRecord[] {
  return getPhotosForDevice(pack, deviceId);
}

export function getDeviceCompliance(
  pack: Segpack,
  deviceId: string
): ComplianceRecord[] {
  return getComplianceForDevice(pack, deviceId);
}

export function getOpenFaults(pack: Segpack): FaultRecord[] {
  return getFaults(pack).filter(
    (item) =>
      item.status !== "resolved" &&
      item.status !== "closed" &&
      item.status !== "not-a-fault"
  );
}

export function getResolvedFaults(pack: Segpack): FaultRecord[] {
  return getFaults(pack).filter(
    (item) => item.status === "resolved" || item.status === "closed"
  );
}

export function getFaultRemedialActions(
  pack: Segpack,
  faultId: string
): RemedialAction[] {
  return ensureArray(getFaultById(pack, faultId)?.recommendedActions);
}

export function getFaultTestResults(
  pack: Segpack,
  faultId: string
): NonNullable<FaultRecord["testResults"]> {
  return ensureArray(getFaultById(pack, faultId)?.testResults);
}

export function getPackStats(pack: Segpack) {
  const systems = getSystems(pack);
  const faults = getFaults(pack);
  const parts = getParts(pack);
  const replacements = getReplacements(pack);
  const compliance = getComplianceRecords(pack);
  const photos = getPhotos(pack);
  const markups = getMarkups(pack);
  const devices = getAllDevices(pack);

  return {
    systems: systems.length,
    devices: devices.length,
    faults: faults.length,
    openFaults: getOpenFaults(pack).length,
    resolvedFaults: getResolvedFaults(pack).length,
    parts: parts.length,
    replacements: replacements.length,
    compliance: compliance.length,
    photos: photos.length,
    markups: markups.length,
  };
}