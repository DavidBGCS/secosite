...
export type SegpackTextReportOptions = {
  includeManifest?: boolean;
  includeSite?: boolean;
  includeSystems?: boolean;
  includeFaults?: boolean;
  includeResolvedFaults?: boolean;
...
const DEFAULT_OPTIONS: Required<SegpackTextReportOptions> = {
...
function yesNoUnknown(value: unknown): string | undefined {
...
function formatLabelValue(label: string, value: unknown): string | undefined {
  const text = clean(value);
  return text ? `${label}: ${text}` : undefined;
}

function formatArrayLine(label: string, values: Array<string | undefined> | undefined): string | undefined {
  if (!Array.isArray(values)) return undefined;
  const filtered = values.map(clean).filter((v): v is string => Boolean(v));
  if (filtered.length === 0) return undefined;
  return `${label}: ${filtered.join(", ")}`;
}

function formatSiteAddress(address: Segpack["site"]["address"]): string | undefined {
...
function formatSystemLine(system: InstalledSystem): string {
  const bits = [
    system.name,
...
function formatFaultDetailLines(
  pack: Segpack,
  fault: FaultRecord,
  lineBreak: string
): string[] {
  ...
  const locationText =
    clean(fault.location?.locationText) ??
  ...
  if (actions.length) {
    ...
    for (const action of actions) {
      ...
      if (action.outcome) lines.push(`      Outcome: ${action.outcome}`);
    }
  }
  ...
  if (tests.length) {
    ...
    for (const test of tests) {
      ...
      if (test.notes) lines.push(`      Notes: ${test.notes}`);
    }
  }
...
function formatComplianceLine(item: ComplianceRecord, relatedSystemName?: string): string {
  const bits = [
    item.title,
...
function section(title: string, bodyLines: string[], lineBreak: string): string {
  if (bodyLines.length === 0) return "";
  return [title, ...bodyLines].join(lineBreak);
}

export function getSegpackTextReport(
  pack: Segpack,
  options: SegpackTextReportOptions = {}
): string {
  ...
  if (opts.includeSystems) {
    ...
    const lines = systems.length
      ? systems.flatMap((system) => {
          const row: string[] = [formatSystemLine(system)];
  ...
  if (opts.includeCompliance) {
    ...
    const lines = items.length
      ? items.flatMap((item) => {
          ...
          const row: string[] = [formatComplianceLine(item, systemName)];
          if (item.summary) row.push(`  Summary: ${item.summary}`);
          if (item.finding) row.push(`  Finding: ${item.finding}`);
          if (item.implication) row.push(`  Implication: ${item.implication}`);
...
export function getSegpackShortTextSummary(pack: Segpack): string {
  const summary = getSegpackSummaryReport(pack);
