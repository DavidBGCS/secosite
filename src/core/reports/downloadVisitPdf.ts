...
function renderLongSection(title: string, value?: string): string {
  if (!value) return "";
  return `
...
function buildHtml(siteFile: SiteFile, visitId: string): string {
            ...
            ${view.linkedReplacements
              .map(
                (replacement) => `
                  <tr>
                    <td>
                      <div class="table-main">${escapeHtml(
                        replacement.title ??
      ...
      <div class="topbar">
        ...
        <div>
          ...
          <div class="report-title-sub">
            ${escapeHtml(view.visit.visitId)}${
              view.visit.completedAt
...
export async function downloadVisitPdf(
  ...
  const root = container.querySelector("#visit-pdf-root") as HTMLElement | null;
  if (!root) {
